...
import co.nodb.sol.bean.Bbean;
import co.nodb.sol.service.EMSInformationService;
import co.nodb.sol.service.StudentAllSelectService;
import co.nodb.sol.service.StudentModifyService;
import co.nodb.sol.service.StudentRegisterService;
import co.nodb.sol.service.StudentSelectService;
...
public class MainClassUseXMLsImport {
	...
	public static void main(String[] args) {
		...
		EMSInformationService informationService = ctx.getBean("info",EMSInformationService.class);
		informationService.outputEMSInformation();
		
		StudentRegisterService registerService = ctx.getBean(StudentRegisterService.class);
		for (int j = 0; j < sNums.length; j++) {
			Student student = new Student();
			student.setsNum(sNums[j]);
			student.setsId(sIds[j]);
			student.setsPw(sPws[j]);
			student.setsName(sNames[j]);
			student.setsAge(sAges[j]);
			student.setsGender(sGenders[j]);
			student.setsMajor(sMajors[j]);
			registerService.register(student);
		}
		
		StudentModifyService modifyService  = ctx.getBean(StudentModifyService.class);
		modifyService.modify(new Student("H39lesvj7544vf89", "deer", "00000", "melissa", 
				26, "W", "Vocal Music"));
		
		StudentSelectService selectService = ctx.getBean(StudentSelectService.class);
		...
		StudentAllSelectService allSelectService = ctx.getBean(StudentAllSelectService.class);
